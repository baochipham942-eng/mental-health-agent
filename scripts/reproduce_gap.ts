...
inputs.forEach(text => {
  ...
  // Test parseImpactScore directly (it is in scope)
  const score = parseImpactScore(text);
  console.log(`  > parseImpactScore result: ${score}`);
  
  // Test detectGap
  const gap = detectGap("init", text);
  console.log(`  > detectGap result: hasGap=${gap.hasGap} key=${gap.hasGap ? gap.gapKey : 'NONE'} impactScore=${gap.intake.impactScore}`);
});
